...
| `video_v1_real_v1.weights.json` | `nrc_real_01`（nrc 真机示范第二轮） | 59 | **37.50%** | 首次 nrc 真机加训；**非可用模型**（见 §11.15） |
...
- **方向类在单帧观测下本质不可分**：`up/down/left/right` 的区别**在帧间、不在帧内**，
  单帧灰度图看不出角色「正在往上走还是往下走」——所有配方都没能把 `up`（12 个训练
  样本）学起来，全挤在 `press`/`down`。这是**架构上限**，调超参解决不了；
  要学方向必须换观测（帧差 / 帧对 / 短时序）。
