...
## ✅ 补记（2026-09-15）：两个缺口已闭合，新档远程 10 局出数据

上面 §一 盘点的「**缺口有 3 个**」中的前两个（终态校验、批量证据）在本日全部补齐：

| 缺口 | 状态 | 证据 |
|---|---|---|
| ① 终态校验（脚本侧独立抓帧） | **✅ 已落地并按新口径验收** | 终态判据改为「终态帧胜负标记 == plan 判胜步结论」⇒ **一致性 10/10**；`.workbuddy/sparkle/remote/check_verdict_consistency.py` 可离线重算任一批次 |
| ② 批量证据 | **✅ 新档 10 局** | `batch_remote.sh 10 1`（远程 MuMu 模拟器，2026-09-15 09:36–09:58）：**5 胜 5 负 = 50%**，终态一致性 **10/10**，大字判据分离 **≈5.5 倍** |
| ③ 失败可归因到具体 step 与判据 | **✅** | 5 局 LOSE 均止于第 12 步（判胜步 `strict` 失败 → 退出码 1），归因到**内容**而非执行（动作失败 0 次） |

**基线更新**：旧「20 局 30%」出自**旧档且含假 WINNER**，仅作历史记录、**不再作当前基线**。
当前基线 = **胜率 50%（新档 13 局累计 ≈46%）· 终态一致性 100% · 判据分离 ≥5 倍**。

**判据②（≥80%）仍不满足，但收尾口径不变**：胜负在时间上**成块出现**（5 连胜后 5 连败）、
主题每局都在换，而点击位置固定 ⇒ 胜负由**内容**决定，属决策层。**本线不必再刷局数** ——
再加局数只会更精确地测出同一个分布。

**新增一条红线（本轮踩到）**：**跑批进行中不要编辑跑批脚本**。bash 按**字节偏移**增量读取
正在执行的脚本，中途改动会让后续轮次读到错位内容（本批第 10 轮即如此；数据完好，
用离线脚本重算即可）。另：日志截断**别用 `cut -c`**（按字节切中文会产生乱码）。

**逐轮明细与留证**：`outputs/game-sensei-sparkle跑批报告.md` §六~§九；
方法学结论同见 `docs/plan-sparkle.md` §六。
