...
**默认路径：原图留磁盘 → Go 判读命令优先调用根目录 `.env` 配置的渠道和模型（按配置顺序尝试）→ 渠道模型失败后使用本地 Ollama 兜底 → 两路均失败才裁剪、压缩后交对话模型。全程主对话默认只接收文字证据。**

“原图留本地”指文件存放位置，不代表优先调用本地模型；禁止将旧的“Ollama 优先”规则套用到本项目。

#### A. 原图由独立Go 判读命令识别（渠道优先、本地兜底，禁止先读进主对话）
...
3. **图片识别统一走正式 Go 命令 `go run ./cmd/vlm -in <图片> -question "<问题>"`（可编译复用），配置统一在项目根目录 `.env`（已 gitignore，勿上传；不得在代码中硬编码密钥、渠道地址或模型）**。实现放 `internal/imagejudge`，不再使用 `.workbuddy/nrc/vlm.py` 临时入口。参数及配置见 `docs/teacher-demo-video.md` §9.3。
   ...
   （逗号分隔、按优先级自动兜底）。默认 `-backend auto` 在渠道模型全部失败后自动调用本地 Ollama；`-backend ollama` 仅用于单独诊断。本地地址读取 `OLLAMA_BASE_URL` 或 `OLLAMA_PORT`，模型读取 `OLLAMA_MODEL`，不硬编码端口或模型；模型或服务变更时改 `.env`，不改代码。
4. Go 判读命令从磁盘读取原图，将图片编码后直接发给 VLM 服务；**命令输出只包含文字结果**，
...
6. 小字模糊、关键字段缺失、结论矛盾或答非所问：先从原图无损裁剪目标区域，补必要上下文和具体问题，
   裁剪后最多再试 1 次；服务不可达、模型不支持视觉、超时或显存不足也属于明确失败。Go 命令自动完成渠道候选到 Ollama 的失败切换，不直接把图片读入主对话。Ollama 仍无法回答时，记录两路失败原因后走 B 节路径。
   原图超过服务请求限制或模型资源限制时可裁剪或适配尺寸，但保留原图并记录处理；不要反复发送同一输入。语义不正确由调用方核验，不能把非空输出当成答案正确。
